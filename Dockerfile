...
RUN apt-get update
RUN apt-get install -y wget curl git unrar-free cron php php-curl mc tzdata nginx
RUN apt-get clean
...
RUN mv /nod_mirror/nod32ms.conf /var/www/eset_mirror_script/ && mv /nod_mirror/nginx_config_default /etc/nginx/sites-available/default
RUN rm -r /nod_mirror
